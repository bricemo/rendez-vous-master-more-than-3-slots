...
( function( $ ) {
	...
	media.view.RendezVous = wp.media.View.extend( {
		...
		events: {
			...
			'change .rdv-input-when' : 'storeWhenInput', // Modified on 1/29/2018 by @awijasa. Previously: 'blur .rdv-input-when' : 'storeWhenInput',
		...
		refresh: function() {
			...
			if ( ! disabled ) {
				_.each( this.controller.state().get( 'rdvdays' ).models, function( model ) {
					if( ( model.attributes.hour1 + model.attributes.hour2 + model.attributes.hour3 + model.attributes.hour4 + model.attributes.hour5 + model.attributes.hour6 + model.attributes.hour7 + model.attributes.hour8 ).length > 3 )
						disabled = false;
						days = true;
				} );
			}
